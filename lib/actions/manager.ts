...
export async function getManagerStats(branchId: string, dateStr?: string) {
  ...
  if (stats.length === 0) {
    return {
      ...
      periodGrossTransferTotal: debtPaymentTransfer + posTransferReceived,
      periodDebtTotal: 0,
      periodExpensesTotal,
      totalDebt,
  ...
  return {
    ...
    periodGrossTransferTotal: (stats[0].periodTransferTotal || 0) + debtPaymentTransfer + posTransferReceived,
